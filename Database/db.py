...
class Database:
    ...
    async def insert_message_to_db(self, message_id, needVotes):
        ...
        results = await self.fetch_messages_from_db(self)
        return results
    ...
    def fetch_rankings_in_guild(self, guildId):
        self.cursor.execute( f"""SELECT RankingName, RankingID from Rankings WHERE GuildID='{ctx.guild.id}'""")
        return self.cursor.fetchall()

    @classmethod
    def create_new_ranking(self, name, guildId):
        self.cursor.execute( f"""INSERT INTO Rankings (RankingName, GuildID) VALUES ('{name}','{guildId}')""")
        self.connection.commit()

    @classmethod
    def fetch_rankingIds(self, guildId):
        self.cursor.execute(f"""SELECT RankingID from Rankings WHERE GuildID='{ctx.guild.id}'""")
        return self.cursor.fetchall()

    @classmethod
    def fetch_rankingIds(self, guildId, rankingName):
        self.cursor.execute(f"""SELECT RankingID from Rankings WHERE GuildID='{ctx.guild.id} AND RankingName='{rankingName}'""")
        return self.cursor.fetchall()
    
    @classmethod
    def fetch_user_from_points(self, userToDb, rankingId):
        self.cursor.execute( f"""SELECT User from Points WHERE User='{userToDb}' AND RankingID='{rankingID}'""")
        return self.cursor.fetchall()
    
    @classmethod
    def insert_user_to_points(self, userToDb, rankingId):
        self.cursor.execute( f"""INSERT INTO Points VALUES('{userToDb}', {rankingID} , {int("0")})""")
        self.connection.commit()
    
    @classmethod
    def increase_total_memebers_in_ranking(self, rankingId):
        self.cursor.execute(f"""UPDATE Rankings SET NumberOfMembers = NumberOfMembers + 1 where RankingID = {rankingID};""")
        self.connection.commit()

    @classmethod
    def fetch_ranking_name(self, rankingName):
        self.cursor.execute(f"""SELECT RankingName from Rankings WHERE RankingName='{rankingName}'""")
        return self.cursor.fetchall()

    @classmethod
    def fetch_user_with_points(self, rankingName, guildId):
        self.cursor.execute(f"""SELECT p.User, p.Points from Points p INNER JOIN Rankings r ON r.RankingID = p.RankingID WHERE r.RankingName='{rankingName}' AND r.GuildID={guildId}""")
        return self.cursor.fetchall()
